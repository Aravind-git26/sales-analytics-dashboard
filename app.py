...
with c1:
    ...
    fig1 = px.bar(cat_sales, x='Category', y=['Sales','Profit'], title='🛍️ Sales vs Profit by Category', barmode='group', color_discrete_sequence=['#636EFA','#00CC96'])
...
fig7 = px.bar(top_states, x='State', y='Sales', title='🏙️ Top 10 States by Sales', color='Sales', color_continuous_scale='Blues')
fig7.update_layout(xaxis_tickangle=45)
st.plotly_chart(fig7, use_container_width=True)

st.markdown("---")
heatmap = filtered_df.pivot_table(values='Profit', index='Category', columns='Region', aggfunc='sum').fillna(0)
fig8 = px.imshow(heatmap, title='🌡️ Profit Heatmap — Category vs Region', color_continuous_scale='RdYlGn', aspect='auto')
